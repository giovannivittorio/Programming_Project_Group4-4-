...
(If the congress rate is larger than the market rate we compute the difference as ***\|net return - market return\|***, otherwise we calculate the difference as ***-\|market return - net return\|*** )
...
```{r line_plot_prep, include=FALSE}
...
reshaped_monthly_returns |>
  ...
  annotate("text", 
           x = as.Date("2020-03-01"),  y = 500, label = "COVID 19",  hjust = 1.1, 
...
```{r map_pplot_prep, include=FALSE}
...
```{r map_visualisation_2_prep, message=FALSE, warning=FALSE, include=FALSE}
####################################################
...
Grouping the congress members by state then calculating the factor by which each state out-performed (under-performed) the market rate helps visualize if abnormal returns are spatially concentrated or if the trend is more widespread. A nationwide abnormal return pattern could indicate that congress members are capable of systematically achieving abnormal returns, in turn pointing to insider trading.
